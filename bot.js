...
// Sql
const DadaDB = config.DATABASE.define('Dada', {
    info: {
...
async function Dada () {
    ...
    var StrSes_Db = await DadaDB.findAll({
        where: {
          info: 'StringSession'
        }
    });
    ...
    conn.on ('credentials-updated', async () => {
        ...
        if (StrSes_Db.length < 1) {
            await DadaDB.create({ info: "StringSession", value: Session.createStringSession(authInfo) });
        } else {
...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp...')}`);
    ...
    conn.on('open', async () => {
        ...
        console.log(
            chalk.green.bold('✅️DADA working ' + config.WORKTYPE + ' 𝚗𝚘𝚠 '));
          // thanks to afnanplk
...
*/
                        try {
                            ...
                            if (config.NOLOG == 'off') {
                                ...
                                await conn.sendMessage(conn.user.jid, '*~_________~ DADA ~______~*' +
